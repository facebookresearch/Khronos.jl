...
function place_adjoint_source(
    ...
    if length(frequencies) == 1
        ...
        # For per-voxel SourceData injection in Khronos, include_resolution=true:
        # Khronos's source convention (D += S, no dt factor) requires the dV
        # volume factor to properly weight each voxel's contribution.
        # Note: Meep's FourierFields uses include_resolution=False because
        # Meep's source injection already includes dt and fourier_sourcedata
        # handles the spatial discretization differently.
        scale = adj_src_scale(
