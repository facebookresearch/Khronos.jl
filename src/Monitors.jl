...
function init_monitors(sim::SimulationData, monitor::TimeMonitor)
    ...
    gv = GridVolume(sim, Volume(center=monitor.center,size=monitor.size), monitor.component)
...
function init_monitors(sim::SimulationData, monitor::DFTMonitor)
    ...
    for i_3 = axes(scale, 3)
        for i_2 = axes(scale, 2)
            for i_1 = axes(scale, 1)
                point = grid_volume_idx_to_point(sim, gv, [i_1, i_2, i_3])
                scale[i_1, i_2] = compute_interpolation_weight(
                    point, vol, sim.ndims, sim.Δx, sim.Δy, sim.Δz
    ...
    monitor.monitor_data = DFTMonitorData{backend_array, complex_backend_array}(
        ...
        complex_backend_array(
            ...
            zeros(
                (get_gridvolume_dims(gv))..., 
                length(monitor.frequencies)
                )
            ),
        scale,
        gv,
...
@kernel function update_dft_monitor!(
    ...
    for k in eachindex(frequencies)
        monitor_fields[ix, iy, iz, k] += (
            exp(frequencies[k] * time_fac) * sim_fields[ix + offset_x, iy + offset_y, iz + offset_z]
        )
    end
end
