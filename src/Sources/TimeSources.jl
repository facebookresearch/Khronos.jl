...
TBW
"""
last_source_time(sim::SimulationData) = maximum([last_source_time(src) for src in sim.sources])
...
function eval_time_source(src::GaussianPulseData{numType}, t::Real) where numType<:Number
    ...
    return exp(-tt * tt / (2 * src.width * src.width)) * exp( -2 * pi * im * src.fcen * tt) * amp
end
