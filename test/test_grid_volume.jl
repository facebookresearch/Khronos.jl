...
@testset "3D GridVolumes" begin
    ...
    # point gv
    # --------------------------------------------------- #
    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , 0. , 0. ]), Khronos.Ex())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [2, 1, 1])
    @test all(gv.start_idx .≈ [40, 41, 41])
    @test all(gv.end_idx .≈ [41, 41, 41])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , 0. , 0. ]), Khronos.Ey())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [1, 2, 1])
    @test all(gv.start_idx .≈ [41, 40, 41])
    @test all(gv.end_idx .≈ [41, 41, 41])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , 0. , 0. ]), Khronos.Ez())
    ...
    # line gv
    # --------------------------------------------------- #
    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , 0. ]), Khronos.Ex())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [2, 81, 1])
    @test all(gv.start_idx .≈ [40, 1, 41])
    @test all(gv.end_idx .≈ [41, 81, 41])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , 0. ]), Khronos.Ey())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [1, 80, 1])
    @test all(gv.start_idx .≈ [41, 1, 41])
    @test all(gv.end_idx .≈ [41, 80, 41])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , 0. ]), Khronos.Ez())
    ...
    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , Inf ]), Khronos.Ex())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [2, 81, 81])
    @test all(gv.start_idx .≈ [40, 1, 1])
    @test all(gv.end_idx .≈ [41, 81, 81])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , Inf ]), Khronos.Ey())
    
    @test all([gv.Nx, gv.Ny, gv.Nz] .≈ [1, 80, 81])
    @test all(gv.start_idx .≈ [41, 1, 1])
    @test all(gv.end_idx .≈ [41, 80, 81])

    gv = Khronos.GridVolume(sim, Khronos.Volume(center=[0. , 0. , 0.],size=[0. , Inf , Inf ]), Khronos.Ez())
