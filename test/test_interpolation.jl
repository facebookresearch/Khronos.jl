...
function restrict_to_array!(arr, volume)
    for (iy, y_val) in enumerate(y)
        for (ix, x_val) in enumerate(x)
            arr[ix, iy] = Khronos.compute_interpolation_weight(
                [x_val, y_val, 0.0],
                volume,
                ndims,
                Δx,
...
@testset "Subpixel-volume restrictions" begin
    ...
    for ix = -1.6:0.4:1.6
        line_x_volume =
            Khronos.Volume(center = [0.14, ix * Δy, 0], size = [sx * Δx, sy * Δy, 0])
        restrict_to_array!(w, line_x_volume)
    ...
    for ix = -0.8:0.4:0.8
        line_x_volume =
            Khronos.Volume(center = [ix * Δx, 0.14, 0], size = [sx * Δx, sy * Δy, 0])
        restrict_to_array!(w, line_x_volume)
        @test sum(w) ≈ sx * sy atol = 1e-14
    end
end
